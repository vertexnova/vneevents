...
- **.clang-format** and **.clang-tidy** (with **.clang-tidy** at repo root) are enforced in CI; focus review on design and logic. If you spot style/naming that automation might miss, flag it.

### Project layout

- Library: **src/vertexnova/events/** — Public API: **include/vertexnova/events/** — Tests: **tests/** — Examples: **examples/**
- Dependencies: **deps/internal/** (vnecommon, vnelogging), **deps/external/** (e.g. googletest, glfw). No top-level `external/` or `libs/`.
...
- Missing or incorrect thread-safety documentation for public APIs on the event/input surface.
- Unnecessary edits under **deps/external/** or **deps/internal/** unless the PR intentionally updates or configures them.
