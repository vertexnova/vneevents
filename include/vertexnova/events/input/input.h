...
class Input {
   public:
    ...
    [[nodiscard]] static bool isKeyJustPressed(int key) { return InputManager::isKeyJustPressed(key); }

    [[nodiscard]] static bool isKeyJustReleased(int key) { return InputManager::isKeyJustReleased(key); }

    // Mouse buttons
    [[nodiscard]] static bool isMouseButtonPressed(int button) { return InputManager::isMouseButtonPressed(button); }
    ...
    [[nodiscard]] static std::pair<int, int> mousePosition() { return InputManager::mousePosition(); }

    [[nodiscard]] static std::pair<float, float> mouseScroll() { return InputManager::mouseScroll(); }

    // Window size
    [[nodiscard]] static std::pair<int, int> windowSize() { return InputManager::windowSize(); }

    // Per-frame reset
    static void nextFrame() { InputManager::nextFrame(); }
    ...
    static void updateMousePosition(int x, int y) { InputManager::instance().inputState().updateMousePosition(x, y); }
