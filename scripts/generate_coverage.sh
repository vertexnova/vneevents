...
#==============================================================================
# Coverage Report Generation Script for VneEvents
...
usage() {
    ...
    echo ""
    echo "Generate code coverage reports for VneEvents"
    echo ""
    echo "Options:"
    ...
    echo "  -f, --filter-src-only     Filter to only src/vertexnova/events (exclude tests, 3rd_party)"
    ...
    echo "  $0 -d build/Debug/build-macos-clang-17.0.0 -e ./bin/TestVneEvents -f"
...
generate_macos_coverage() {
    ...
    if [[ "$FILTER_SRC_ONLY" == true ]]; then
        ...
        xcrun llvm-cov report "${summary_args[@]}" 2>&1 | grep -E "^src/vertexnova/events|^TOTAL" | head -20
    else
...
generate_linux_coverage() {
    ...
    if [[ "$FILTER_SRC_ONLY" == true ]]; then
        filter_args+=(--extract "$coverage_info" 'src/vertexnova/events/*')
    fi
...
# Main execution
echo -e "${BLUE}=== VneEvents Coverage Report Generator ===${NC}"
echo ""
