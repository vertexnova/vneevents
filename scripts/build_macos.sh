...
#
# This script builds VneEvents for macOS with Xcode integration
#==============================================================================
...
interactive_mode() {
  echo "=== VneEvents Interactive Build Configuration ==="
  echo ""
...
build_cmake_command() {
  ...
  if [ "$GENERATE_XCODE" = true ]; then
    base_cmd="cmake -G Xcode -DCMAKE_BUILD_TYPE=$BUILD_TYPE -DCMAKE_C_COMPILER=${CC_PATH} -DCMAKE_CXX_COMPILER=${CXX_PATH} -DCMAKE_OSX_DEPLOYMENT_TARGET=13.3 -DVNEEVENTS_LIB_TYPE=$VNEEVENTS_LIB_TYPE -DBUILD_TESTS=ON -DBUILD_EXAMPLES=ON"
  else
    base_cmd="cmake -DCMAKE_BUILD_TYPE=$BUILD_TYPE -DCMAKE_C_COMPILER=${CC_PATH} -DCMAKE_CXX_COMPILER=${CXX_PATH} -DCMAKE_OSX_DEPLOYMENT_TARGET=13.3 -DVNEEVENTS_LIB_TYPE=$VNEEVENTS_LIB_TYPE -DBUILD_TESTS=ON -DBUILD_EXAMPLES=ON"
  fi
...
if [ "$GENERATE_XCODE" = true ]; then
  BUILD_COMMAND="xcodebuild -project vneevents.xcodeproj -configuration $BUILD_TYPE -parallelizeTargets -jobs $JOBS"
  TEST_COMMAND="xcodebuild -project vneevents.xcodeproj -configuration $BUILD_TYPE -target RUN_TESTS"
else
...
case $ACTION in
  ...
  xcode)
    ...
    echo "You can now open vneevents.xcodeproj in Xcode"
    ;;
  xcode_build)
    if [ "$CLEAN_BUILD" = true ]; then
      clean_build
