...
#==============================================================================
# VneEvents Documentation Generation Script
...
check_prerequisites() {
    ...
    if [[ ! -f "$PROJECT_ROOT/CMakeLists.txt" ]]; then
        log_error "Not in VneEvents project root. Please run from project root."
        exit 1
    fi
...
generate_api_docs() {
    ...
    if [[ -f "$PROJECT_ROOT/docs/doxyfile.in" ]] || [[ -f "$PROJECT_ROOT/Doxyfile" ]]; then
        # Configure CMake with ENABLE_DOXYGEN=ON
        cmake -DENABLE_DOXYGEN=ON "$PROJECT_ROOT" 2>&1 | grep -E "(Doxygen|doxyfile)" || true
        
        # Build the doc_doxygen target
        if cmake --build . --target doc_doxygen 2>&1 | grep -E "(Generating|Doxygen)" || true; then
            log_success "API documentation generated successfully"
        else
            log_warning "API documentation generation may have failed"
        fi
    else
...
# Main function
main() {
    log_info "Starting VneEvents documentation generation..."
    ...
    if [[ "$DOXYGEN_AVAILABLE" == "true" && -d "$BUILD_DIR/docs/html" ]]; then
        log_info "API documentation available at: $BUILD_DIR/docs/html/index.html"
    fi
}

# Help function
show_help() {
    echo "VneEvents Documentation Generator"
    echo ""
