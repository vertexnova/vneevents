#!/usr/bin/env python3
"""
VneEvents Cppcheck Analyzer

A script to run cppcheck static analysis on C/C++ source files with
VneEvents-specific configuration and filter out third-party warnings.
...
Examples:
    python scripts/cppcheck_analyzer.py src/vneevents
    python scripts/cppcheck_analyzer.py src/vneevents --verbose
    python scripts/cppcheck_analyzer.py --file src/vertexnova/events/events.cpp
"""
...
"""
    ...
    <div class="summary">
        ...
        <p>Great! No static analysis issues were detected in the VneEvents code.</p>
    </div>
"""
...
def run_cppcheck(files: list, folder_name: str, verbose: bool = False, save_raw_xml: bool = False) -> bool:
    ...
    # Add include paths for VneEvents
    cmd.extend([
    ...
    try:
        ...
        if raw_report.exists():
            ...
            print(f"Found {total_issues} total issues in VneEvents code (excluding third-party)")
...
Examples:
  ...
  python scripts/cppcheck_analyzer.py src/vneevents --save-raw-xml
        """
    )
    ...
    print("VneEvents Cppcheck Analyzer")
    print("=" * 40)
    print()
